...
function getOptions(inputOpts) {
    try {
        const params = {};
        if(inputOpts){
            params.argv = inputOpts;
        }

        opts = cli(options, params);
        if (!opts.server) {
...
function writeNPMRC(data) {
    return checkIfNpmrcExists()
        ...
        .then((existing) => fsp.writeFile(opts.path, [existing, data].join('\n'), {
            encoding: 'ascii'
        }))
        .then(() =>{
